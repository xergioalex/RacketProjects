...
;Estas funciónes se encarga de pintar el área de graficación.
...
;              (if  (or  
;                        (and (>= (+ 295 (* valor-x 20)) (posn-x limite_2)) (>= (+ 285 (* (posn-y punto_xy) 20) ) (posn-y limite_2)))
...
(define (convertir->numero cadena contador continuar)
      (if (char? cadena)
          (- (char->integer cadena) 48)
          (if (= continuar 1)         
              (if (= contador (string-length cadena))
                  0
                  (+  (* (- (char->integer (string-ref cadena contador)) 48) (expt 10 (- (string-length cadena) contador 1))) (convertir->numero cadena (+ contador 1) 1))
              )
              (if  (eqv? (string-ref cadena contador) #\/)
                 (begin
                     (display (substring cadena 0 (+ contador 1)))
                     (newline)
                     (display (substring cadena (+ contador 1) (string-length cadena)))
                     (/ (convertir->numero (substring cadena 0 contador ) 0 1) (convertir->numero (substring cadena (+ contador 1) (string-length cadena)) 0 1))
                  )
                 (if (= contador (- (string-length cadena) 1))
                   (convertir->numero cadena 0 1)
                   (convertir->numero cadena (+ contador 1) 0)
                 )
             )
          )
      )
...
(define (convertir->cadena numero cifras continuar)
      (if (and (< numero 0) (= continuar 0))
          (string-append  "-" (convertir->cadena (* numero -1) cifras continuar))
          (if (> (expt 10 cifras) numero)
              (if (< numero 0)
                  (make-string 0)
                  (if (< numero 10)
                      (make-string 1 (integer->char (+ numero 48)))  
                      (string-append  (make-string 1 (integer->char (+ (/ (- numero (remainder numero (expt 10 (- cifras 1)))) (expt 10 (- cifras 1))) 48))) 
                                      (convertir->cadena (remainder numero (expt 10 (- cifras 1))) (- cifras 1) 1))  
                  )
              )
              (convertir->cadena numero (+ cifras 1) continuar)            
         )
     )
)
...
(define (ordenar variable grado vector_coeficiente vector_exponente vector_coefi_ordenado vector_exp_ordenado posicion_vectores posicion_vec)
      (if (= grado -1)
          (begin
                ...
                ;(integrar vector_coefi_ordenado vector_exp_ordenado 0)
          )
...
(define (establecer_signos polinomio variable  grado vector_coeficiente vector_exponente posicion_pol posicion_vec)
        (if (= posicion_pol (string-length polinomio))
            ...
            (if (or (and (= posicion_pol 0) (and (>= (char->integer (string-ref polinomio posicion_pol)) 48) (<= (char->integer (string-ref polinomio posicion_pol)) 57)))
               ...
               (begin
                    (vector-set! vector_exponente posicion_vec (convertir->numero (vector-ref vector_exponente posicion_vec) 0 0))
                    (vector-set! vector_coeficiente posicion_vec (convertir->numero (vector-ref vector_coeficiente posicion_vec) 0 0)) 
              ...
              (if (eqv? (string-ref polinomio posicion_pol) #\-)
                   (begin
                      (vector-set! vector_exponente posicion_vec (* (convertir->numero (vector-ref vector_exponente posicion_vec) 0 0) 1))
                      (vector-set! vector_coeficiente posicion_vec (* (convertir->numero (vector-ref vector_coeficiente posicion_vec) 0 0) -1))
                      (establecer_signos polinomio variable grado vector_coeficiente vector_exponente (+ posicion_pol 1) (+ posicion_vec 1))
                   )
...
(define (verificar polinomio memoria_variable contador variable_seguida tamaño-vector)
    (if (string? polinomio)  
        ...
        (cond ((> variable_seguida 1)
              ...
              ((or (eqv? (string-ref polinomio contador) #\/) 
...
(define (escribir teclear pos_x pos_y polinomio contador)
        ...
        (cond ( (not (char? caracter)) (inicio pos_x pos_y polinomio contador) )
              ...
              ((= (char->integer caracter) 13) 
                   (verificar (ignorar_retroceso polinomio 0 (make-string 0)) 0 0 0 0)
              )
...
;---------------------------------------------------------INTRO-----------------------------------------------
;En esta parte va la introduccion
...
(define (barra_carga posicion_x)
  (if (> posicion_x 412)
      ...
      (begin
        ...
        (tiempo_espera posicion_x 1 300000)
      )          
  )
)
(barra_carga 184)